...
// --- Background work monitor (Direction B) ---

const STILL_RUNNING_SIGNAL = "[still-running]";
const BG_POLL_INTERVAL_MS = 30_000;
const BG_MAX_WAIT_MS = 10 * 60_000;

function looksLikeBackgroundWork(text: string): boolean {
  return /background|run.{0,10}in.{0,10}background|subagent|background.{0,10}agent|後台|背景任務|will.{0,20}notify|通知.*完成|完成後.*通知/i.test(text);
}

async function monitorBackgroundWork(
  token: string,
  chatId: number,
  ...
  const typingInterval = setInterval(() => sendTyping(token, chatId, threadId), 4000);

  try {
    while (Date.now() - startTime < BG_MAX_WAIT_MS) {
      await new Promise((resolve) => setTimeout(resolve, BG_POLL_INTERVAL_MS));

      const probe = `[system: background-status-check] 你的背景任務完成了嗎？如果完成了請直接給我結果，如果還在跑請只回覆 ${STILL_RUNNING_SIGNAL}`;
      let result: Awaited<ReturnType<typeof runUserMessage>>;
      try {
        result = await runUserMessage("telegram-bg-poll", probe);
      } catch (err) {
        console.warn(`[Telegram] Background poll error for ${label}:`, err);
        continue;
      }

      if (result.exitCode !== 0) {
        console.warn(`[Telegram] Background poll failed (exit ${result.exitCode}) for ${label}`);
        continue;
      }

      const text = (result.stdout ?? "").trim();
      if (!text || text.includes(STILL_RUNNING_SIGNAL)) {
        console.log(`[Telegram] Background task still running for ${label}`);
        continue;
      }

      const { cleanedText } = extractReactionDirective(text);
      await sendMessage(token, chatId, cleanedText || text, threadId);
      return;
    }

    console.warn(`[Telegram] Background monitor timed out for ${label}`);
    await sendMessage(token, chatId, "⏱ 背景任務已超過 10 分鐘，請重新詢問進度。", threadId);
  } finally {
...
async function handleMessage(message: TelegramMessage): Promise<void> {
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      // Keep typing alive if Claude started background work
      if (looksLikeBackgroundWork(cleanedText || "")) {
        clearInterval(typingInterval);
        await monitorBackgroundWork(config.token, chatId, threadId, label);
      }
    }
  } catch (err) {
